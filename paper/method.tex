...
\begin{theorem}[Strong Completeness and Uniqueness of the Geodesic Path]
	...
	\end{equation*}
	Assume the Riemannian metric is given in \eqref{eq:inner_product} and the length of a path is defined in \eqref{eq:length}. Suppose that the unique geodesic path connecting \(\overline{\mathbf{x}}\) and \(\mathbf{x}\) exists. Then,
	\begin{equation*}
		\sum_{i=1}^n \Bigl| A_i^{\gamma}(\mathbf{x}) \Bigr| = \Bigl| f(\mathbf{x})-f(\overline{\mathbf{x}}) \Bigr|
	\end{equation*}
	if and only if \(\gamma\) is this unique geodesic path.
\end{theorem}

\begin{proof}
	We sketch the proof in two parts.
	
	\paragraph{Necessity:} Assume
\begin{equation*}
	\sum_{i=1}^n \Bigl| A_i^{\gamma}(\mathbf{x}) \Bigr| = \Bigl| f(\mathbf{x})-f(\overline{\mathbf{x}}) \Bigr|.
\end{equation*}
	By the chain rule,
...
\end{equation*}
	Taking absolute values and using the triangle inequality (both for the sum and the integral) yields
\begin{equation*}
...
\end{equation*}
	Since the attributions also satisfy
\begin{equation*}
	\sum_{i=1}^n \Bigl| A_i^{\gamma}(\mathbf{x}) \Bigr|\le \int_0^1 \sum_{i=1}^n \left|\frac{\partial f}{\partial x_i}(\gamma(t))\,\dot{\gamma}_i(t)\right| dt,
\end{equation*}
	the assumed equality forces equality in each triangle inequality. In particular, for almost every \(t\) all terms \(\frac{\partial f}{\partial x_i}(\gamma(t))\,\dot{\gamma}_i(t)\) share the same sign so that
\begin{equation*}
...
\end{equation*}
	Thus, \(\gamma\) minimises \(L(\gamma)\); by uniqueness of the geodesic, \(\gamma\) must be the geodesic.
	
	\paragraph{Sufficiency:} Conversely, if \(\gamma\) is the unique geodesic connecting \(\overline{\mathbf{x}}\) and \(\mathbf{x}\), its minimising nature ensures that the instantaneous contributions are sign-aligned, so that for almost every \(t\)
\begin{equation*}
	\sqrt{\sum_{i=1}^n \Bigl(\frac{\partial f}{\partial x_i}(\gamma(t))\,\dot{\gamma}_i(t)\Bigr)^2} = \sum_{i=1}^n \left|\frac{\partial f}{\partial x_i}(\gamma(t))\,\dot{\gamma}_i(t)\right|.
\end{equation*}
	Reversing the above arguments then gives
\begin{equation*}
	\sum_{i=1}^n \Bigl| A_i^{\gamma}(\mathbf{x}) \Bigr| = \Bigl| f(\mathbf{x})-f(\overline{\mathbf{x}}) \Bigr|.
\end{equation*}

	Thus, the strong completeness condition holds if and only if \(\gamma\) is the unique geodesic.
\end{proof}

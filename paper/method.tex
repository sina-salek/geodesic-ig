...
\citep[Theorem 1]{sundararajan2017axiomatic} shows that IG is the only path method that satisfies symmetry for any function. However, as noted in \cite{kapishnikov2021guided}, while straight path is the only one satisfying symmetry for any function, given a specific function, it may be possible to find other paths that satisfy Symmetry. We generalise this theorem for Geodesic IG on Riemannian manifolds and how that our method satisfies Symetry for the neural network function we sample the paths for.

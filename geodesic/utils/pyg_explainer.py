...
def set_masks(
    model: torch.nn.Module, mask: Tensor, edge_index: Tensor, apply_sigmoid: bool = True
):
...
def to_captum(
    model: torch.nn.Module, mask_type: str = "edge", output_idx: Optional[int] = None
) -> torch.nn.Module:
...
class Explainer(torch.nn.Module):
    ...
    def get_initial_prediction(
        self, x: Tensor, edge_index: Tensor, batch: Optional[Tensor] = None, **kwargs
    ):
    ...
    def get_loss(
        self, out: Tensor, prediction: Tensor, node_idx: Optional[int] = None, **kwargs
    ):
        if self.return_type == "regression":

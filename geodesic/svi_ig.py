...
class SVI_IG(GradientAttribution):
    ...
    def _get_straight_line(
        self, inputs: Tensor, baselines: Tensor, n_steps: int
    ) -> Tensor:
        """Helper function to compute the straight line path once."""
        alphas = torch.linspace(0, 1, steps=n_steps, device=inputs.device).view(
            -1, 1, 1
        )
    ...
    def _get_approx_paths(
        ...
        def edge_weight(p1, p2):
            ...
            grads = torch.autograd.grad(
                outputs.sum(), path_points, create_graph=True, retain_graph=True
            )[0]
        ...
        for i in range(len(inputs)):
            ...
            all_points = np.vstack(
                [aug_np, baselines_np[i : i + 1], inputs_np[i : i + 1]]
            )  # Changed order: baseline first, then input
            ...
            start_node = len(
                aug_np
            )  # Index of baseline point (first after augmentation)
            end_node = (
                len(aug_np) + 1
            )  # Index of input point (second after augmentation)
            ...
            try:
                ...
                path_length = np.cumsum(
                    np.linalg.norm(np.diff(path_points, axis=0), axis=1)
                )
                ...
                try:
                    ...
                    print(
                        f"Interpolation failed for sample {i}, using straight line. Error: {e}"
                    )
        ...
        paths = paths.transpose(
            0, 1
        )  # Transpose to get [n_steps, batch_size, features]

        assert torch.allclose(
            paths[0], baselines
        ), f"Paths don't start at baselines. Got {paths[0]} vs {baselines}"
        assert torch.allclose(
            paths[-1], inputs
        ), f"Paths don't end at inputs. Got {paths[-1]} vs {inputs}"

        return paths

    def potential_energy(
        self, path: Tensor, initial_paths: Tensor, beta: float
    ) -> Tensor:
        """
    ...
    def model(
        self, inputs: Tensor, baselines: Tensor, initial_paths: Tensor, beta: float
    ):
        ...
        delta = pyro.sample(
            "path_delta",
            dist.Normal(torch.zeros_like(initial_paths), 1.0).to_event(
                initial_paths.dim()
            ),
        )
    ...
    def guide(
        ...
        delta_loc = pyro.param("delta_loc", lambda: torch.zeros_like(initial_paths))

        # Zero out the location parameters at start and end points
        delta_loc = (
            delta_loc * (1 - (alphas == 0).float()) * (1 - (alphas == 1).float())
        )
    ...
    def _optimize_paths(
        ...
        if augmentation_data is not None:
            ...
            initial_paths = self._get_straight_line(
                input_tensor, baseline_tensor, self.n_steps
            )
            current_beta = beta
            decay_rate = 1
        ...
        assert torch.allclose(
            optimized_path[0], baseline_tensor, rtol=rtol, atol=atol
        ), "Path doesn't start at baseline"
        assert torch.allclose(
            optimized_path[-1], input_tensor, rtol=rtol, atol=atol
